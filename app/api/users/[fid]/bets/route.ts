...
/**
 * GET /api/users/[fid]/bets
 * Get all bets for a user with statistics (blockchain read + cache)
 * Query params:
...
export async function GET(
  ...
  try {
    ...
    // Fetch market details and claimable winnings for each bet
    const betsWithDetails = await Promise.all(
      cachedBets.map(async (bet) => {
        ...
        // Get claimable winnings for this market
        let claimableWinnings = '0';
        try {
          const winnings = await getClaimableWinnings(
            BigInt(bet.marketId),
            walletAddress as Address
          );
          claimableWinnings = winnings.toString();
        } catch (error) {
          console.error(`Error fetching winnings for market ${bet.marketId}:`, error);
        }
    ...
    const validBets = betsWithDetails.filter((bet) => bet !== null);
    ...
    const totalWagered = validBets.reduce(
      (sum, bet) => sum + BigInt(bet?.amount || '0'),
      0n
    );
    
    const resolvedBets = validBets.filter((bet) => bet?.market?.status === 2);
    const wonBets = resolvedBets.filter((bet) => {
      const market = bet?.market;
      if (!market) return false;
      const winningOutcome = market.outcomes.find(
        (o) => o.outcomeId === Number(market.status === 2 ? bet.outcomeId : -1)
      );
      return winningOutcome !== undefined;
    });
    
    const totalClaimable = validBets.reduce(
      (sum, bet) => sum + BigInt(bet?.claimableWinnings || '0'),
      0n
    );
      ...
      data: {
        ...
        statistics: {
          ...
          winRate: resolvedBets.length > 0 
            ? ((wonBets.length / resolvedBets.length) * 100).toFixed(2) 
            : '0',
